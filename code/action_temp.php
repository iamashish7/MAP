...
$conn2 = new mysqli("localhost", "ashish", "ashish007", "HPC5");
// Check connection
...
switch ($chart) {
    case "1":
        ...
        $sql = "select jobid,start,end from Master_E where id>=0 and date >= '" . $from . "' and date <= '" . $to . "'";
    ...
    case "2":
        ...
        $sql = "select concat(year(date),'-',month(date)) as d, count(distinct(jobid)) as jobs from Master_Q where id>=0 and date >= '" . $from . "' and date <= '" . $to . "' group by d";
        $sql2 = "select concat(year(date),'-',month(date)) as d, count(distinct(jobid)) as jobs from Master_E where id>=0 and date >= '" . $from . "' and date <= '" . $to . "' group by d";
        $timeend = round(microtime(true) * 1000000);
        //echo '<script>console.log('.($timeend-$timebegin).')</script>';
        // echo '<script>console.log('.($timebegin).')</script>';
        // echo '<script>console.log('.($timeend).')</script>';
    ...
    case "3":
        ...
        $json_array = array();
        $timebegin = round(microtime(true) * 1000000);
        $sql = "select count(distinct jobid) as total from loganalysis_master";
        $sql2 = "select status,count(distinct jobid) as c from loganalysis_master group by status";
        $timeend = round(microtime(true) * 1000000);
        //echo '<script>console.log('.($timeend-$timebegin).')</script>';
    ...
    case "4":
        ...
        $sql = "select queue, avg(CAST((UNIX_TIMESTAMP(start)-UNIX_TIMESTAMP(ctime))/(60*60) as UNSIGNED)) as avg_waitingtime from Master_S where queue not like '"."R%"."' and start >= ctime and ctime>='".$from."' and ctime<='".$to."' group by queue";
        $timeend = round(microtime(true) * 1000000);
        //echo '<script>console.log('.($timeend-$timebegin).')</script>';
        // echo '<script>console.log('.($timebegin).')</script>';
        // echo '<script>console.log('.($timeend).')</script>';
    ...
    case "6":
        $json_array = [];
    	$timebegin = round(microtime(true) * 1000000);
        $sql = "select timestampdiff(hour,ctime,start) as wtime,date,count(*) as jobs from Master_E where id>=0 and date >= '" . $from . "' and date <= '" . $to . "' and timestampdiff(hour,ctime,start) >= 0 group by wtime,date order by date,wtime;";   
        $timeend = round(microtime(true) * 1000000);
        //echo '<script>console.log('.($timeend-$timebegin).')</script>';
        // echo '<script>console.log('.($timebegin).')</script>';
        // echo '<script>console.log('.($timeend).')</script>';
    ...
    case "7":
        ...
        $timebegin = round(microtime(true) * 1000000);
        $sql = "select id,timestampdiff(hour,ctime,start) as wtime from Master_E where id>=0 and queue not like 'R%' and start>'1970-01-01 00' and timestampdiff(hour,ctime,start)>=0 and date >= '" . $from . "' and date <= '" . $to . "' ";
        $timeend = round(microtime(true) * 1000000);
        //echo '<script>console.log('.($timeend-$timebegin).')</script>';
        // echo '<script>console.log('.($timebegin).')</script>';
        // echo '<script>console.log('.($timeend).')</script>';
