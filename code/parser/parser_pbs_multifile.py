...
used to insert complete logs of HPC2010 to database SavedLogs as table name HPC2010 with schema given below

+--------------+--------------+------+-----+---------+----------------+
| Field        | Type         | Null | Key | Default | Extra          |
+--------------+--------------+------+-----+---------+----------------+
| jobid        | int(50)      | NO   | PRI | NULL    | auto_increment |
| jobid_real   | varchar(50)  | YES  |     | NULL    |                |
...
| wtime        | int(50)      | YES  |     | -1      |                |
| rtime        | int(50)      | YES  |     | -1      |                |
| proc_alloc   | int(50)      | YES  |     | -1      |                |
| avg_cpu_time | int(50)      | YES  |     | -1      |                |
| mem_used     | int(50)      | YES  |     | -1      |                |
| req_proc     | int(50)      | YES  |     | -1      |                |
| req_rtime    | int(50)      | YES  |     | -1      |                |
| req_mem      | int(50)      | YES  |     | -1      |                |
| status       | varchar(50)  | YES  |     | NULL    |                |
| uid          | varchar(50)  | YES  |     | NoUser  |                |
| gid          | varchar(50)  | YES  |     | NoGroup |                |
| exe_app_num  | int(50)      | YES  |     | -1      |                |
| queue        | varchar(100) | YES  |     | NULL    |                |
| part         | int(50)      | YES  |     | -1      |                |
| prec_job_num | int(50)      | YES  |     | -1      |                |
| think_time   | int(50)      | YES  |     | -1      |                |
+--------------+--------------+------+-----+---------+----------------+
"""
...
jobid_pattern = re.compile(r'[0-9]+\[[0-9]*\]')
workq_pattern = re.compile(r'workq')
count = 0
count_neg = 0
id_D = 0
id_R = 0
...
def record_E(row):
    ...
    for r in temp:
        ...
        if(len(ele)<2):
            continue
        d[ele[0]] = ele[1]
    if(workq_pattern.search(d['queue'])!=None or jobid_pattern.search(row[2])!=None):
        return
    if(row[2] in exitA_in_run):
        ...
        table['rtime'] = str(int(toSeconds(d['resources_used.walltime']))) if ('resources_used.walltime' in d.keys()) else str(int(d['end']) - int(d['start']))
