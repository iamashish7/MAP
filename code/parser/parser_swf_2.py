...
start_time_epoch = (start_time - datetime.datetime(1970,1,1)).total_seconds()
for line in lines:
    entry = {}
    values = line.split()
    if (len(values)==18):
        ...
        entry["stime"] = str(start_time_epoch + int(values[1]))
        ...
        entry["start"] = str(start_time_epoch + int(values[1]) + int(values[2]))
        entry["end"] = str(start_time_epoch + (int(values[1])+int(values[2])+int(values[3])))
        insertData(entry)
closeConnection()
print (print_str, end ="")
