...
def setup():
    createDB()
    table_query = "create table if not exists "+ tableName +" (jobid int(50),date varchar(100),stime varchar(100),start varchar(100),end varchar(100),wtime int(50),rtime int(50),proc_alloc int(50),avg_cpu_time int(50),mem_used int(50),req_proc int(50),req_rtime int(50),req_mem int(50),status varchar(50),uid int(50),gid int(50),exe_app_num int(50),queue varchar(100),part int(50),prec_job_num int(50),think_time int(50),PRIMARY KEY (jobid));"
...
for line in lines:
    ...
    if(len(res)>0):
        ...
        print_str += '#'+start_time.strftime('%Y-%m-%d')
        break
...
for line in lines:
    ...
    if(len(res)>0):
        ...
        timezone = matcher.group(4)
        year = matcher.group(5)
        end_time = datetime.datetime.strptime(timestamp, '%a %b %d %H:%M:%S '+timezone+' %Y')
        print_str += '#'+end_time.strftime('%Y-%m-%d')
        break
